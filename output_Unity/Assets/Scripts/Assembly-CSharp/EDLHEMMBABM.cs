...
public class EDLHEMMBABM : global::FOLJNEPEKCA<EDLHEMMBABM>
{
	...
	public void OnWin(bool InNeedWait = true)
	{
		...
		WinUI.newBoxProgress = WinUI.boxCompleted ? FPFGGCMEDND.LEVELS_PER_BOX : FPFGGCMEDND.Instance.CurLevelBoxProgress;
	...
	private void ShowLevelStartPops()
	{
	...
	private bool ShowNewItemPopIfNeeded()
	{
		...
		foreach (POJCEPBNNIP itemType in itemTypes)
		{
			...
			JEFOMCDAPGK.Instance.MarkNewItemPopShown(itemType);
			NewItemPop.popType = itemType;
			// 录屏：关卡对象出生完成约0.4s后弹出（进关后约1.0s）
	...
	{
		LDNJKLHLHAH.Instance.StartQueue(false, OGPEIPAJIDB.MainLevelWin);
	}
	...
	public void OpenLoseUI()
	{
		...
		{
			MCCIJBJGMCK.UnlockOnce();
			MgrUI.Instance.Open("coreplaywin/LoseUI");
		});
	}
	...
	public bool AddOne(bool InConsumeItem = true)
	{
		...
		if (InConsumeItem)
		{
			GNEJJHEDEBL.Instance.UseItem(POJCEPBNNIP.Extra, 1, DPOCDHPGHMF.Extra, 0f, TotalItems.Count);
		}
	...
	public bool Undo(bool InConsumeItem = true)
	{
		...
		if (InConsumeItem)
		{
			GNEJJHEDEBL.Instance.UseItem(POJCEPBNNIP.Undo, 1, DPOCDHPGHMF.Undo, 0f, TotalItems.Count);
		}
	...
	public bool Magic(bool InConsumeItem = true)
	{
		...
		if (InConsumeItem)
		{
			GNEJJHEDEBL.Instance.UseItem(POJCEPBNNIP.Magic, 1, DPOCDHPGHMF.Magic, 0f, TotalItems.Count);
		}
	...
	public bool Shuffle(bool InConsumeItem = true)
	{
		...
		if (InConsumeItem)
		{
			GNEJJHEDEBL.Instance.UseItem(POJCEPBNNIP.Shuffle, 1, DPOCDHPGHMF.Shuffle, 0f, TotalItems.Count);
		}
