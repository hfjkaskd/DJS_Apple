...
public class NewPlayerGuider : MonoBehaviour
{
	...
	// CorePlay 位于 Bottom 层（基准0，自用0-20），Mid 弹窗从1000起；遮罩取100盖住场景但低于弹窗
	private const int MASK_ORDER = 100;

	private const int HIGHLIGHT_ORDER = 101;

	private const int FRONT_ORDER = 102;
	...
	private void Awake()
	{
		...
		BringToFront(m_GuideFinger);
		if (m_GuideText != null)
		{
			BringToFront(m_GuideText.transform.parent != null ? m_GuideText.transform.parent.gameObject : m_GuideText.gameObject);
		}
	}

	/// <summary>让节点渲染在引导遮罩之上。</summary>
	private static void BringToFront(GameObject go)
	{
		if (go == null)
		{
			return;
		...
		canvas.sortingOrder = FRONT_ORDER;
	}
	...
	private void ShowMask(bool show)
	{
		if (m_MaskCanvas != null)
		{
			...
			m_MaskCanvas.sortingOrder = MASK_ORDER;
		}
		if (m_MaskBG == null)
		{
			return;
