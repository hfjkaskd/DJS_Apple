...
namespace CorePlay
{
	...
	public class CorePlayUI : BaseUI
	{
		...
		protected override void Init()
		{
			...
			{
				m_CommonCoinBtn.Init(FCCMMLGODPL.ShopinPlay, true);
			}
			InitAllItemBtn();
		}
		...
		public void UpdateBehavior()
		{
			...
			if (m_CommonCoinBtn != null)
			{
				// 录屏：第1关（教学关）不显示金币入口，第2关起显示
				m_CommonCoinBtn.gameObject.SetActive(JEFOMCDAPGK.Instance.CurMainLevelIndex > 1);
		...
		{
			EDLHEMMBABM controller = EDLHEMMBABM.Instance;
			LevelData levelData = JEFOMCDAPGK.Instance.MainLevelData;
			bool canRestore = levelData.hasEffectiveSave && levelData.gameSnapshot != null && levelData.gameSnapshot.levelConfig != null && !JEFOMCDAPGK.Instance.Data.isLevelFinished && !levelData.isLose;
			if (canRestore)
			{
				CoroutineManager.Instance.StartCor(controller.RestoreFromSnapshot(null));
			}
			else
			{
				...
				if (config == null)
				{
					Debug.LogError("[CorePlayUI] 无法获取当前关卡配置");
					return;
				}
				levelData.Reset(false);
				levelData.timerRunning = true;
				CoroutineManager.Instance.StartCor(controller.ParseConfig(config, false, null));
			}
		}
		...
		{
			CorePlayItemBtn btn = GetItemBtn(itemType);
			return btn != null ? btn.transform.position : Vector3.zero;
		}
		...
		{
			MgrUI.Instance.Open("pops/setting/GameSettingUI");
		}
		...
		{
			if (JEFOMCDAPGK.Instance.CurMainLevelIndex != 1)
			{
				return;
			}
